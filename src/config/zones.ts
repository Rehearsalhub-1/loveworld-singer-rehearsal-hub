...
export function getHqGroups(): Zone[] {
  return ZONES.filter(zone => isHQGroup(zone.id))
}
...
export function isHQGroup(zoneId: string | undefined): boolean {
  ...
  const withHyphen = clean.includes('-') ? clean : clean.replace(/^zone(\d+)$/, 'zone-$1');
  return (
    HQ_GROUP_IDS.includes(zoneId) ||
    HQ_GROUP_IDS.includes(clean) ||
    HQ_GROUP_IDS.includes(withHyphen) ||
    HQ_GROUP_IDS.some(id => id.replace(/-/g, '') === withoutHyphen) ||
    clean === 'hq' ||
    clean === 'loveworld-singers-hq'
  );
}
